...
//    	char buf[50];
		...
		uint8_t tx_data[2];
//    	sprintf(buf, "%x \r\n", request);
//    	_UART_Transmit(&huart2, (uint8_t*)buf, strlen(buf), 100);

        //readSensorData();

		// Send back temperature data if requested
		if (request == 0x01) {
			tx_data[0] = (uint8_t)temperature; // Send the 8-bit temperature value
			tx_data[1] = 0;
			HAL_GPIO_WritePin(LD3_GPIO_Port, LD3_Pin, 1);
		}
		// Send back humidity data if requested
		else if (request == 0x02) {
			tx_data[0] = (uint8_t)humidity; // Send the 8-bit humidity value
			tx_data[1] = 0;
